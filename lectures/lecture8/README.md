...
Такой класс называется обобщенным классом. Под термином обобщение следует понимать "применимость к большой группе классов". Идентификатор `T` между угловыми скобками `<>` называется **типизированной переменной**. Он укзаывает, что класс `Box` требует тип Java в качестве аргумента, чтобы сделать его полным. Имя `T` является произвольным, но существуют соглашения (они описаны в конце лекции). В этом случае типизированная переменная `T` представляет тип элементов, который мы хотим хранить в коробке.
