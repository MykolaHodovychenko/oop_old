...
#### ⚠️**Механизм функциональных интерфейсов и лямбда-выражений позволяет легко передавать поведение как аргумент метода. С помощью передачи поведения, мы можем
#### писать гибкие методы, работа которых может меняться в зависимости от передаваемого в метод поведения.**
